...
| Local embeddings tier | LIVE ✅ (08-10) | LM Studio removed from box (Jay, deliberate cleanup). Ollama `nomic-embed-text` now serves retrieval embeddings, CPU-pinned (`num_gpu:0`) — `OLLAMA_VULKAN=1` set on this box, RX580=display GPU, same TDR-crash risk as LM Studio, avoided entirely since the model is tiny. `corpus_docs` index rebuilt: 1,788 docs, 1,060 far_rfo now embedded. |
| Local generation tier | ⚠️ DARK (since 08-10) — **leave it dark, don't rewire without reading this** | Was LM Studio-served (`gemma-4-e4b`, `llm_client.py` port 1234) — now gone with LM Studio. Cascade falls to Venice (already proven reliable — daily scans have run on Venice for days). Ollama already has `gemma4:12b` pulled, and it'd be tempting to just rewire `llm_client.py` to it — **don't, not without addressing the hardware issue first.** `[[hardware-crash-under-local-compute]]` documents a worse-than-expected 2026-07-21 finding: sustained local generation (15-40+ min) crashed the box three times, *including a CPU-only run* — the "CPU-only is safe" conclusion from the June GPU-TDR forensics does not hold for heavy generation, only for short/light jobs (confirmed again 08-10: the embedding swap below is a ~2-minute job over a 137M-param model, not comparable). Prime suspect is a marginal/failing PSU, not software. Local generation stays off until that's physically addressed — Venice-only is the correct posture, not a stopgap to "fix" opportunistically. |
