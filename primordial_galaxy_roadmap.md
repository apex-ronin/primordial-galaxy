...
| Local generation tier | ⚠️ DARK (since 08-10) | Was LM Studio-served (`gemma-4-e4b`, `llm_client.py` port 1234) — now gone with LM Studio. Cascade falls to Venice (already proven reliable — daily scans have run on Venice for days). Ollama already has `gemma4:12b` pulled; rewiring `llm_client.py` to it is a fast-follow, not urgent since Venice covers it, but local-primary architecture is currently generation-optional until this is done. |
